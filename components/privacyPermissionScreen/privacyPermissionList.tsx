...
import { usePrivacyPermissionContext } from '@/contexts/privacyPermission'
...
export function PrivacyPermissionListing() {
  const { privacyQuery } = usePrivacyPermissionContext()
        ...
        <Card variant="outline" key={i}>
          <HStack space="lg" className="items-start">
            <Box className={cn('rounded-full bg-background-200 p-3', { 'bg-primary-200': item.require })}>
              <Icon
                as={() => <PrivacyIcon id={item.id} />}
                className={cn({ 'text-primary-500': item.require })}
                size="xl"
              />
            </Box>
